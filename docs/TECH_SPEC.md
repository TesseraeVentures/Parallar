...
**Rules:** position commitments, position_root, registry state, settled flags → **persistent storage, never temporary**; extend TTLs on access; instruments carry rent implications for their lifetime (a 10-year bond's vault must not quietly archive). MVP: persistent + extend-on-access is sufficient. Production: TTL monitoring + scheduled extension is an operational requirement (PRODUCTION_GAP G9). **Enforced:** storage-type review in code review checklist; CLAUDE.md invariant.
