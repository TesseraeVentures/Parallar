...
### 10.6 RPC event retention (24h default / 7d max)
**Fact:** stellar-rpc retains events for only ~**24 hours by default** (7 days is the *maximum* configurable/queryable window) — so even a one-day-old epoch can be unqueryable on a default node, and monthly/quarterly coupon epochs always outrun it. Deeper history requires a continuously-ingesting indexer (write `getEvents` into your own store), a self-hosted/archive Horizon, a commercial archive-RPC, or Hubble/Galexie history-archive ingestion.
**Rule:** `history_builder` must not assume default-RPC completeness — and does not: settlement consumes a precomputed `Scan` via a pluggable `DataSource` (today `FileSource` = an archive export / RPC dump captured during the window; a live continuous-ingest source is the production swap, **not yet built** — fine for fresh demo windows, not to be presented as production-complete). The strongest answer is the **`credit_v2`/`credit_v3` issuer attestation (G1/G4)**: the issuer signs the snapshot, so deep-history access is unnecessary at settlement — authenticity comes from the signature, not from re-deriving history off RPC. Settlement of an old epoch must remain possible forever. **Enforced:** history_builder abstraction + the attestation layer.
