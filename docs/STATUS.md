...
## 2026-06-16 (cont.) — R61: Stellar integration sweep (vs the official ZK references) + founder Qs

A 6-agent sweep WebFetched the official Stellar ZK docs (CAP-0074/0075, the ZK-on-Stellar page, the SDK BN254/Poseidon migration docs, the Nethermind verifier, Protocol 25/26 guides, RPC docs, the ZK-proofs skill) and reconciled each against our build. **Verdict: INTEGRATES — built on the correct primitives; no blockers; verifies on Protocol 26 today.**
- **Confirmed correct:** on-chain Groth16 via the BN254 pairing host fn (CAP-0074, Final/P25, live on P26) — standard 4-pair equation; journal committed as its sha256 digest; 260-byte selector-wrapped seal (`73c457ba`); Poseidon2 runs IN-GUEST (the recommended pattern), zkhash HorizenLabs BN254-Fr t=3/d=5/RF=8/RP=56 = the canonical CAP-0075 param set; on-chain path is sha256 + BN254 only (zero Poseidon host calls in contracts/). Vendored verifier matches latest upstream **commit e8ff6ea (2026-04-11), parameters.json v3.0.0, selector 73c457ba, soroban-sdk 25.1.0** (pinned per CLAUDE.md rule #6).
- **Applied now (honesty/positioning):** corrected the RPC-retention figure (TECH_SPEC §10.6: **24h default / 7d max**, not "~7 days"; noted FileSource-only today + the attestation answer); README now cites CAP-0074/0075 Final + a "Relation to the Stellar privacy stack" note (custom commitment, not Confidential Tokens/Privacy Pools — pre-empts the reviewer question); DEPLOYMENT docs subdomain corrected to NEVER publish PRODUCTION_GAP/STATUS.
- **Founder's RPC question answered:** not a problem by design — settlement consumes a precomputed Scan (pluggable DataSource), and credit_v2/v3 attestation removes the deep-history dependency entirely. For real monthly/quarterly epochs, run a continuous indexer OR use the attested feed (the live continuous-ingest DataSource is the one production piece NOT yet built — fine for the demo).
- **POST-DEMO should-fix checklist (none are P26 blockers):** (1) bump soroban-sdk 25.3.0→26.0.1 in the workspace + prover/host, re-lock, and rename the verifier's `crypto::bn254::Fr`→`Bn254Fr` (deprecated alias in 26.x) — ship the freeze on 25.3.1 (verifies on P26; on the patched side of CVE-2026-32322 + the verify path has no Fr==Fr compare), bump after recording; (2) re-run the onchain_verify test against a LIVE P26 testnet RPC before deploy; (3) document the Poseidon2 param tuple as the on-chain re-impl contract. **Watch:** Protocol 27 ('Zipper') hits testnet 2026-06-18 (during the event) — no evidence it changes the ZK host fns, but confirm the verifier WASM + env.crypto().bn254() still load on P27 before the final recording.
- **Blocked:** none.

## 2026-06-17 — R71: confidential-cover keeper/sequencer (G3 critical path) — toward the full confidential pBOND product

Founder steered to the FULL confidential pBOND product (Product Vision doc; confidential cover the default, interactive dApp, ~12 days, no shortcuts; proofs via Rosetta-x86 + friendbot). The unblocker is the G3 keeper.
- **`prover/host/src/keeper.rs`** — the single-writer sequencer over one confidential instrument's hidden cover aggregate. Holds the opening `(total, salt)`; `plan_buy` advances it + binds the buyer's position; `plan_withdraw` proves the unchanged aggregate still fits post-withdrawal collateral. Pure construction (native-testable); the real Groth16 is `prove_solvency_buy/withdraw` (x86/Rosetta). Trust model = documented G3 (keeper is a trusted operator; private to the market/chain). **4 native tests pass** (advance, chained-sequencer-invariant, insolvent-rejected, withdraw-floor).
